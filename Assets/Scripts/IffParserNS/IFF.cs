...
namespace IffParserNS
{
    ...
    public class IFF : IDisposable
    {
        ...
        public UnityEngine.Color32[] CMAP { get; private set; }
        public BMHD BMHD { get; private set; }
        public VERS VERS { get; private set; }

        private Color32[] Bitmap { get; set; }
        ...
        private void ReadBODY(BinaryReader reader, int chunkLength)
        {
            ...
            Bitmap = new UnityEngine.Color32[BMHD.H * BMHD.W];
            ...
            for (int iLine = 0; iLine < BMHD.H; iLine++)
            {
                ...
                for (int i = 0; i < linePixels.Length; i++)
                {
                    ...
                    Bitmap[(iLine * BMHD.W) +  i] = ColorIndexToColor(iColor);
                }
            }

            Texture2D = new Texture2D(BMHD.W, BMHD.H, GraphicsFormat.R8G8B8A8_UNorm, TextureCreationFlags.None);
            Texture2D.SetPixels32(Bitmap);
            Texture2D.Apply(true, false);
        }

        protected virtual UnityEngine.Color ColorIndexToColor(UInt32 colorIndex)
        {
            if (CMAP == null)
        ...
        {
            CMAP = new Color32[chunkLength / 3];
            for (var i = 0; i < chunkLength / 3; i++)
            {
                ...
                CMAP[i] = new Color32((byte)r, (byte)g, (byte)b, 255);
            }
        }
